    ...
    <div className="h-[calc(100vh-8.5rem)] flex flex-col bg-zinc-900/20 border border-zinc-800/60 rounded-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="px-6 py-4 border-b border-zinc-800 bg-zinc-900/40 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-theme-accent-bg flex items-center justify-center border border-theme-accent-border shadow-md">
            <Brain className="h-5 w-5 text-theme-accent" />
          </div>
          <div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.map((msg, idx) => (
          ...
          >
            {msg.role !== "user" && (
              <div className="h-8 w-8 rounded-lg bg-theme-accent-bg border border-theme-accent-border flex items-center justify-center shrink-0">
                <Brain className="h-4 w-4 text-theme-accent" />
              </div>
            )}
            
            <div className={`max-w-[70%] rounded-2xl px-4 py-3 text-xs leading-relaxed space-y-2 ${
              msg.role === "user" 
                ? "bg-theme-btn text-white rounded-tr-none" 
        ...
        {isLoading && (
          <div className="flex gap-4 justify-start">
            <div className="h-8 w-8 rounded-lg bg-theme-accent-bg border border-theme-accent-border flex items-center justify-center shrink-0">
              <Brain className="h-4 w-4 text-theme-accent animate-pulse" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {quickPrompts.map((p) => {
              ...
              return (
                ...
                >
                  <Icon className="h-4 w-4 text-theme-accent shrink-0" />
      ...
      <div className="p-4 border-t border-zinc-800 bg-zinc-950/40 flex items-center gap-3">
        <input 
          ...
          className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-xs focus:outline-none border-theme-accent-focus text-zinc-300 transition-colors placeholder:text-zinc-655"
          value={input}
        ...
        <button 
          ...
          className="bg-theme-btn text-white h-9 w-9 rounded-xl flex items-center justify-center transition-colors shadow-lg shadow-theme-btn shrink-0"
        >
